...
/*
 * KS FFScouter Call Guard v1.1.3 RELEASE
...
(() => {
  ...
  function normalizeSharedClaims(payload) {
    ...
    for (const [rawTargetId, rawQueue] of Object.entries(faction)) {
      ...
      if (!validTargetId(targetId) || !Array.isArray(rawQueue) || rawQueue.length === 0) return null;
      ...
      if (queue.length !== rawQueue.length) return null;
