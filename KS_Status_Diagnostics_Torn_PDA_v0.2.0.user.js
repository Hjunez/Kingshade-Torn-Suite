...
// - Makes one faction/basic request and a small number of public user diagnostic requests only when "Run diagnostic" is pressed.
...
(() => {
    ...
    function rawMemberEntries(payload) {
        if (!payload?.members) return [];
        if (Array.isArray(payload.members)) {
            return payload.members.map(member => [member?.id ?? member?.player_id, member]);
        }
    ...
    function pickStatusSample(entries, stateName) {
        ...
        for (const [rawId, member] of entries) {
            ...
            if (state === target) {
                return {
                    id: Number(member?.id ?? member?.player_id ?? rawId),
                    name: String(member?.name ?? member?.player_name ?? ""),
                    factionStatus: sanitize(member?.status),
                    lastAction: sanitize(member?.last_action)
                };
            }
        }
        return null;
    ...
    async function queryUserSelections(sample, key, setStatus) {
        if (!sample?.id) return null;

        const base = `${API_BASE}/user/${sample.id}`;
        const common = {
            key,
        ...
        setStatus(`Checking public user/basic for ${sample.name || sample.id}…`);
        const basicQuery = new URLSearchParams({ ...common, selections: "basic" });
        const basic = await requestJson(`${base}?${basicQuery}`, "user/basic");

        setStatus(`Checking public user/profile for ${sample.name || sample.id}…`);
        const profileQuery = new URLSearchParams({
            ...common,
            timestamp: String(Date.now() + 1),
            selections: "profile"
        });
        const profile = await requestJson(`${base}?${profileQuery}`, "user/profile");
        ...
        return {
            ...
            factionStatus: sample.factionStatus,
            factionLastAction: sample.lastAction,
            userBasic: basic,
            userProfile: profile
        };
    }
    ...
    async function buildReport(setStatus) {
        ...
        const entries = rawMemberEntries(payload);
        const hospitalSample = pickStatusSample(entries, "Hospital");
        const travelingSample = pickStatusSample(entries, "Traveling");
        const abroadSample = pickStatusSample(entries, "Abroad");

        const selectedTravelSample = travelingSample || abroadSample;
        const userChecks = {
            hospital: hospitalSample
                ? await queryUserSelections(hospitalSample, key, setStatus)
                : null,
            travelingOrAbroad: selectedTravelSample
                ? await queryUserSelections(selectedTravelSample, key, setStatus)
                : null
        };
        ...
        const report = {
            ...
            purpose: "Verify whether exact Hospital and Traveling/Abroad end times are exposed by faction/basic, public user/basic, public user/profile, or the visible faction DOM.",
            page: {
            ...
            request: {
                endpointFamily: "Torn API diagnostic",
                requestedFactionId: factionId,
                factionHttpStatus: response.status,
                apiKeyIncludedInReport: false
            },
            factionSamples: {
                hospital: hospitalSample,
                traveling: travelingSample,
                abroad: abroadSample
            },
            publicUserEndpointChecks: userChecks,
            visibleFactionRows: {
                selectorCount: rows.length,
                rows: rows.filter(row =>
                    /(?:hospital|traveling|travelling|abroad)/i.test(row.rowText)
                )
            }
        };
    ...
    function openPanel(root) {
        ...
        panel.innerHTML = `
            ...
            <div class="ksd-help">
                The report compares faction/basic with public user/basic and user/profile for one Hospital and one Traveling/Abroad member, plus the visible row data.
