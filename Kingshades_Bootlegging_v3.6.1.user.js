...
(() => {
    ...
    /*
     * Kingshade's Bootlegging v3.6.1
     *
    ...
    const SCRIPT = Object.freeze({
        name: "Kingshade's Bootlegging",
        version: '3.6.1',
        styleId: 'ks-boot-v361-styles',
        panelId: 'ks-boot-v361-panel',
    ...
    function installStyles() {
        ...
        style.textContent = `
            ...
            #${SCRIPT.panelId}.ks-mode-copy {
                ...
                box-shadow: 0 0 6px rgba(74, 163, 255, 0.38) !important;
            }

            #${SCRIPT.panelId}.ks-mode-sell {
                border-color: #45c96f !important;
                box-shadow: 0 0 6px rgba(69, 201, 111, 0.38) !important;
            }

            #${SCRIPT.panelId}.ks-mode-wait {
                border-color: #e0ad35 !important;
                box-shadow: 0 0 6px rgba(224, 173, 53, 0.36) !important;
            }
            ...
            .ks-mode-copy .ks-boot-status-command {
                color: #72b8ff !important;
            }

            .ks-mode-sell .ks-boot-status-command {
                color: #69df8e !important;
            }

            .ks-mode-wait .ks-boot-status-command {
                color: #f0c65e !important;
            }

            .ks-boot-status-detail {
                display: block !important;
    ...
    function determineInstruction(rows) {
        ...
        if (needsRefill.length === 0) {
            return {
                mode: 'sell',
                command: 'SELL DVDS',
                detail: 'Use Sell Counterfeit DVDs until this changes.',
                badge: 'SELL'
            };
        }
        ...
        if (notCoveredByQueue.length === 0) {
            ...
            return {
                ...
                command: 'WAIT',
                detail: waitingFor
                    ? `${waitingFor.genre} is already queued.`
                    : 'Queued copies already cover the refill.',
                badge: 'WAIT'
            };
        }
        ...
        return {
            ...
            detail: `Queue +${remaining}; stop at ${selected.target} total.`,
            badge: `+${remaining}`
        };
    }
    ...
    function renderPanel(buttons, instruction) {
        ...
        main.className = 'ks-boot-status-main';

        const command = document.createElement('span');
        command.className = 'ks-boot-status-command';
        command.textContent = instruction.command;

        const detail = document.createElement('span');
        detail.className = 'ks-boot-status-detail';
        detail.textContent = instruction.detail;

        const badge = document.createElement('span');
        badge.className = 'ks-boot-status-badge';
        badge.textContent = instruction.badge;

        main.append(command, detail);
        panel.append(main, badge);
    }
