...
(() => {
    ...
    function resolveCoreValues(playerId) {
        ...
        const estimateText = normalizeText(sourceEstimate?.bs_estimate_human || cache?.bs_estimate_human || "");
    ...
    function getStatusSnapshot() {
        const direct = window[SCRIPT.sharedCoreKey];
        if (direct?.members && typeof direct.members === "object") return direct;
        const stored = readJson(SCRIPT.sharedStorageKey);
        return stored?.members && typeof stored.members === "object" ? stored : null;
    }

    function sharedMember(playerId) {
    ...
    function resolveStatus(playerId, row) {
        const member = sharedMember(playerId);
        ...
        return {
            ...
            until,
            timerKind,
            description: normalizeText(apiStatus?.description || ""),
            details: normalizeText(apiStatus?.details || ""),
    ...
    function rowInfo(row, originalIndex = 0) {
        ...
        const status = resolveStatus(playerId, row);
    ...
    function removeTimer(row) {
        ...
        row.querySelectorAll(".kswt-status-host").forEach(element => element.classList.remove("kswt-status-host"));
    }
    ...
    function timerTitle(info) {
        ...
        if (info.timerKind === "estimate") {
            const source = info.travel?.estimateSource === "observed-history"
                ? `Estimate from ${Number(info.travel?.historyCount) || 1} observed flight(s)`
                : "Estimate from published route time";
            ...
            return [info.description, source, departure, "Torn exposes no exact arrival timestamp"].filter(Boolean).join(" · ");
        }
        return "";
    }
    ...
    function updateToolbar(infos) {
        ...
        toolbar.querySelector('[data-kswt="status"]').textContent =
            `FF ${counts.core}/${counts.all} · Status ${counts.status}/${counts.all} · ${age === null ? "waiting" : `${age}s old`} · Showing ${infos.filter(matchesFilter).length}/${counts.all}`;
    ...
    function init() {
        ...
        state.observer = new MutationObserver(mutations => {
            ...
            const relevant = mutations.some(mutation => Array.from(mutation.addedNodes).some(relevantMutationNode));
