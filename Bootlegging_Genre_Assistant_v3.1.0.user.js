...
(() => {
    ...
    /*
     * Bootlegging Genre Assistant v3.1.0
     *
    ...
    const state = {
        ...
        originalFetch: null,
        observedFetch: null,
        originalInlineBackgrounds: new WeakMap()
    };
    ...
    function renderGenreButton({
        ...
        if (!state.originalInlineBackgrounds.has(button)) {
            state.originalInlineBackgrounds.set(
                button,
        ...
        applyBackgroundColour(button, diff, maxShortage);
    ...
    function applyBackgroundColour(button, diff, maxShortage) {
        let hue;

        if (diff > 0 && maxShortage > 0) {
            hue = Math.round(
                (1 - diff / maxShortage) * 55
            );
        } else if (diff === 0) {
            hue = 75;
        } else {
    ...
    function renderStockLine(
        ...
        const stockPart =
            queued > 0
                ? CONFIG.showProjectedStock
                    ? `${owned} + ${queued} queued = ${projected}`
                    : `${owned} owned · ${queued} queued`
                : `${owned} owned`;
        ...
        line.textContent = `${stockPart} · ${balancePart}`;
        line.setAttribute(
            'aria-label',
            [
        ...
        document
            ...
            .forEach(button => {
                ...
                const originalBackground =
                    state.originalInlineBackgrounds.get(button);
