...
(() => {
  ...
  const SCRIPT = Object.freeze({
    ...
    version: "1.0.0",
    status: "RELEASE",
    instanceKey: "__ksFFScouterCallGuardV100",
    rowHostPrefix: "ks-ffcg-dibs-v100-",
    ...
    sharedPanelId: "ks-ffcg-shared-v100",
  ...
  function ensureSharedPanel() {
    ...
    shadow.innerHTML = `
      ...
      <div class="panel" data-role="panel" data-state="key-required">
        ...
        <div class="disclosure">Encrypted locally · shared DIBS via FFScouter Hit Calling · no direct Torn API requests</div>
        <div class="policy" data-role="policy">
          ...
          <div class="policy-row"><strong>Data sharing:</strong> DIBS claim/release data goes to FFScouter and is shared with your faction through Hit Calling.</div>
          <div class="policy-row"><strong>Purpose:</strong> faction-war hit coordination and duplicate-call prevention.</div>
          <div class="policy-row"><strong>Key storage & sharing:</strong> encrypted locally by KS; sent only to FFScouter by KS; never sent to Kingshade.</div>
          <div class="policy-row"><strong>Key access:</strong> KS requests no additional Torn permissions; your key must satisfy FFScouter War Room / Hit Calling requirements.</div>
        </div>
      </div>
    `;
  ...
  function classifyTargetState(input) {
    ...
    if (activePlayerId) {
      if (activePlayerId === playerId) {
        return { state: "claimed", seconds: hospitalSeconds, reason: "active-own-dibs" };
      }
      return { state: "blocked", seconds: hospitalSeconds, reason: "another-active-dibs" };
    }

    if (/\bhospital\b/i.test(statusText)) {
      if (hospitalSeconds === null) {
        return { state: "unknown", seconds: null, reason: "hospital-timer-unverifiable" };
      }
      if (hospitalSeconds <= GATE_SECONDS) {
        return { state: "ready", seconds: hospitalSeconds, reason: "hospital-window-open" };
      }
      return { state: "locked", seconds: hospitalSeconds, reason: "hospital-too-early" };
    }

    if (/\bok(?:ay)?\b/i.test(statusText)) {
      return { state: "unavailable", seconds: null, reason: "okay-not-hospital-window" };
    }

    return { state: "unavailable", seconds: null, reason: statusText || "status-unavailable" };
  }
  ...
  function updateDibsControl(host, decision, statusText, sharedClaim = null) {
    ...
    if (pendingTargetId === playerId) {
      ...
      label.textContent = own?.targetId === playerId ? "RELEASING" : "CLAIMING";
      sub.textContent = "WAIT";
      button.setAttribute("aria-label", `${label.textContent} in progress`);
      return;
    }

    if (own?.targetId === playerId) {
      const cleanup = own.cleanupRequired === true;
      button.className = cleanup ? "cleanup" : "claimed";
      button.dataset.state = cleanup ? "cleanup" : "claimed";
      button.dataset.ready = "false";
      button.disabled = !sharedApiKey || sharedWriteBusy;
      label.textContent = cleanup ? "QUEUED" : "DIBBED";
      sub.textContent = "RELEASE";
      button.setAttribute(
        "aria-label",
        cleanup
    ...
    if (sharedClaim) {
      ...
      sub.textContent = extraCount > 0 ? `${firstName} +${extraCount}` : firstName;
      button.setAttribute(
        "aria-label",
        `Shared DIBS held by ${firstName}${extraCount > 0 ? ` with ${extraCount} queued behind` : ""}`
      );
      return;
    }
    ...
    if (state === "blocked") {
      ...
      sub.textContent = own?.claimerName || "ACTIVE";
      button.setAttribute("aria-label", "Blocked by your active shared DIBS on another target");
      return;
    }
    ...
    if (state === "ready") {
      if (!sharedApiKey) {
        button.disabled = true;
        sub.textContent = "SET KEY";
        button.setAttribute("aria-label", "Set FFScouter key before claiming DIBS");
        return;
      }

      button.disabled = sharedWriteBusy;
      button.dataset.ready = sharedWriteBusy ? "false" : "true";
      sub.textContent = Number.isFinite(decision.seconds)
        ? formatCountdown(decision.seconds)
        : "READY";
      button.setAttribute("aria-label", "Claim shared DIBS");
      return;
    }
    ...
    if (state === "locked") {
      ...
      button.setAttribute("aria-label", "DIBS locked until 2 minutes remain");
      return;
    }

    if (state === "unknown") {
      sub.textContent = "UNKNOWN";
      button.setAttribute("aria-label", "Hospital timer could not be verified");
      return;
    }

    sub.textContent = normalizeText(statusText).slice(0, 12).toUpperCase() || "UNAVAILABLE";
    button.setAttribute(
      "aria-label",
      `DIBS unavailable while status is ${normalizeText(statusText) || "unknown"}`
    );
  }
